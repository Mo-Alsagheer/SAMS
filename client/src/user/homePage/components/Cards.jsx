  ...
  useEffect(() => {
    const fetchCommittees = async () => {
      try {
        ...
        toast.error("Failed to load committees. Please try again.");
      } finally {
        setLoading(false);
      }
    };
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 justify-items-center">
        {data.length > 0 ? (
          data.map((item) => (
            <Card
              key={item.id}
              className="relative w-full pt-0 overflow-hidden border-none bg-card transition-all duration-500 ease-in-out shadow-[0_0_25px_rgba(59,130,246,0.3)] hover:-translate-y-3 hover:scale-[1.03] hover:shadow-[0_0_50px_rgba(37,99,235,0.7)]"
            >
              ...
              <CardFooter>
                <Button
                  onClick={() => {
                    toast.info(`Opening ${item.displayTitle} details...`);
          ...
          <div className="col-span-full text-center py-10 text-slate-400">
            No committees found at the moment.
          </div>
        )}
      </div>
    </div>
